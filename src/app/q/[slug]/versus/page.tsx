...
/**
 * Versus mode: endless pairwise matchups for one quiz, vert-measure style.
 * Each pick POSTs /api/vote (which seeds ratings on first vote); the
 * per-quiz Elo leaderboard ranks the answer options against each other.
 */
export default function VersusPage() {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const res = await fetch(`/api/matchup?quiz=${params.slug}`);
    ...
    async (winner: string, loser: string) => {
      ...
      try {
        await fetch("/api/vote", {
          ...
          body: JSON.stringify({ quizSlug: params.slug, winner, loser }),
        });
        await load();
      } finally {
        setVoting(false);
    ...
    <div className="flex min-h-dvh flex-col">
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col items-center justify-center gap-8 px-5 py-10">
        ...
        <div className="flex gap-6 text-[17px]">
          <Link
            href={`/q/${params.slug}/leaderboard`}
